...
async def history(update: types.Message | types.CallbackQuery, subscription_repo: UserSubscriptionRepository, translator: callable):
    ...
    if isinstance(update, types.Message):
        if channels:
            await update.answer(
                translator("history")["channels"],
                reply_markup=generate_history_list_kb(channels)
            )
        else:
            await update.answer(translator("history")["empty"])
...
async def channel_pulse_history_list(callback_query: types.CallbackQuery, callback_data: ChannelPulseHistoryCBData | TurnPageCBData, summary_repo: SummaryRepository, translator: callable):
    ...
    if summaries:
        ...
        await callback_query.message.edit_text(translator("history")["summaries_empty"])
        await callback_query.answer()
...
async def pulse_info(callback_query: types.CallbackQuery, callback_data: PulseInfoCBData, summary_repo: SummaryRepository, translator: callable):
    ...
    await callback_query.message.edit_text(
        f"📌 <b>«{summary.topic}»</b>\n"
        f"{summary.content}\n"
        f"📅 <i>{translator("history")["summary"]}: {summary.summary_date.strftime('%m-%d-%Y')}</i>",
